...
function buildFlags(
  ...
  if (type === 'ir-graph') {
    return ['--allow-natives-syntax', '--trace-turbo', `--trace-turbo-path=${workDir}`];
  }
  const flags = [...TYPE_FLAG_MAP[type]];
  if (type === 'optcode' || type === 'deopts') flags.push('--allow-natives-syntax');
  if (type === 'bytecode') {
    // Definitions-only snippets never CALL their functions; lazy compilation
    // would print nothing. Eager-compile everything.
    //
    // Keep the engine-side filter when the binary advertises it. This is
    // important for large wrappers (the generated prelude can otherwise drown
    // the selected function in unrelated bytecode blocks). V8 versions that
    // ignore the filter are handled by the fallback run in `analyze` below.
    flags.push('--no-lazy');
    if (functionName !== undefined && functionName !== '' && perFunctionFilter) {
...
function hasBytecodeBlock(raw: string, functionName: string | undefined): boolean {
  if (functionName === undefined || functionName === '') return true;
  // V8 13+ appends an SFI pointer to the header. Match the stable name prefix.
  return raw
    .split(/\r?\n/)
    .some((line) => line.startsWith(`[generated bytecode for function: ${functionName}`));
}
...
export class V8EngineAdapter {
  ...
  async analyze(req: AnalysisRequest & { binaryPath: string; timeoutMs?: number }): Promise<AnalysisResult[]> {
    ...
    for (const analysisType of req.analysisTypes) {
      ...
      let flags = buildFlags(analysisType, workDir, req.functionName, caps.perFunctionFilter);
      const startedAt = Date.now();
      let run = await this.runIsolated({
      ...
      // A few V8 builds report per-function-filter support but produce an
      // empty filtered dump for eagerly compiled ESM functions. Retry once
      // without the filter so function analysis remains useful across the
      // supported binary versions; the normalized viewer still focuses the
      // requested block by name.
      if (
        analysisType === 'bytecode' &&
        req.functionName !== undefined &&
        flags.some((flag) => flag.startsWith('--print-bytecode-filter=')) &&
        !hasBytecodeBlock(run.stdout, req.functionName) &&
        !run.timedOut
      ) {
        const fallbackFlags = flags.filter((flag) => !flag.startsWith('--print-bytecode-filter='));
        flags = fallbackFlags;
        run = await this.runIsolated({
          exePath: req.binaryPath,
          args: [...fallbackFlags, entryFile],
          cwd: workDir,
          timeoutMs
        });
      }
